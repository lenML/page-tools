        ...
        pasteNodes: async (wx, wy) => {
          ...
          const newNodes = await Promise.all(clipboard.map(async n => {
            ...
            if (nn.type === 'image' && nn.imageId) {
              ...
              if (blob) return createImageNodeFromBlob(blob, { base: nn, imageId: genId() });
            }
            return nn;
          }));
